...
extern "C" int bprintf(const char *format, ...) {
	va_list list;
	va_start(list, format);
	const int printed = bvsnprintf(nullptr, SIZE_MAX, format, list);
	va_end(list);
	return printed;
}

extern "C" int bserprintf(const char *format, ...) {
...
extern "C" int bsprintf(char *out, const char *format, ...) {
	va_list list;
	va_start(list, format);
	out[0] = '\0';
	const int printed = bvsnprintf(out, SIZE_MAX, format, list);
	va_end(list);
	return printed;
}

extern "C" int bsnprintf(char *out, size_t max, const char *format, ...) {
	va_list list;
	va_start(list, format);
	out[0] = '\0';
	const int printed = bvsnprintf(out, max, format, list);
	va_end(list);
	return printed;
}
...
extern "C" int bvsnprintf(char *out, size_t max, const char *format, va_list list) {
	if (max == 0)
		return 0;
