...
namespace x86_64 {
		...
		for (int i = 0; i < PML4_SIZE / PML4_ENTRY_SIZE; ++i) {
			...
			if (pml4e) {
				...
				if ((pml4e & MMU_PRESENT) && show_pdpt)
					printPDPT(indirect, i_shift, pml4e, show_pdt, pt_display);
			}
		}
		printf_putc = old_putc;
	}

	void PageTableWrapper::printPDPT(bool indirect, size_t i_shift, uint64_t pml4e, bool show_pdt,
	                                 PTDisplay pt_display) {
		for (int j = 0; j < PDPT_SIZE / PDPT_ENTRY_SIZE; ++j) {
			const uintptr_t pmm_offset = indirect? (uintptr_t) Thorn::Kernel::getInstance().physicalMemoryMap : 0;
			const uint64_t &pdpe = ((uint64_t *) ((pml4e + pmm_offset) & ~0xfffL))[j];
			if (pdpe) {
				...
				if (!Thorn::Util::isCanonical(pdpe))
					...
					printPDT(indirect, j_shift, pdpe, pt_display);
			}
		}
	}

	void PageTableWrapper::printPDT(bool indirect, size_t j_shift, uint64_t pdpe, PTDisplay pt_display) {
		for (int k = 0; k < PAGE_DIRECTORY_SIZE / PAGE_DIRECTORY_ENTRY_SIZE; ++k) {
			const uintptr_t pmm_offset = indirect? (uintptr_t) Thorn::Kernel::getInstance().physicalMemoryMap : 0;
			const uint64_t &pde = ((uint64_t *) ((pdpe + pmm_offset) & ~0xfffL))[k];
			if (pde) {
				...
				if (!Thorn::Util::isCanonical(pde))
					...
					if ((pde & MMU_PRESENT) && !(pde & MMU_PDE_TWO_MB))
						printPT(indirect, k_shift, pde);
				} else if (pt_display == PTDisplay::Condensed)
					printCondensed(indirect, k_shift, pde);
			}
		}
	}

	void PageTableWrapper::printPT(bool indirect, size_t k_shift, uint64_t pde) {
		for (int l = 0; l < PAGE_TABLE_SIZE / PAGE_TABLE_ENTRY_SIZE; ++l) {
			const uintptr_t pmm_offset = indirect? (uintptr_t) Thorn::Kernel::getInstance().physicalMemoryMap : 0;
			const uint64_t &pte = ((uint64_t *) ((pde + pmm_offset) & ~0xfff))[l];
			if (pte) {
	...
	void PageTableWrapper::printCondensed(bool indirect, size_t k_shift, uint64_t pde) {
		if (!Thorn::Util::isCanonical(pde)) {
			printf("      Non-canonical.\n");
			return;
		}
		...
		for (int i = 0; i < max; ++i) {
			const uint64_t pte = ((uint64_t *) ((pde + pmm_offset) & ~0xfffL))[i];
			if (pte) {
				if (first == -1)
					first = i;
				last = i;
		...
		if (first == -1) {
			...
			uint64_t first_pte = ((uint64_t *) ((pde + pmm_offset) & ~0xfffL))[first];
			...
			if (first < last) {
				if (first != last - 1) {
					...
					for (int i = first + 1; i < last; ++i)
						if (((uint64_t *) ((pde + pmm_offset) & ~0xfffL))[i])
							++nonempty;
					printf("      ... %d nonempty\n", nonempty);
				}
				uint64_t last_pte = ((uint64_t *) ((pde + pmm_offset) & ~0xfffL))[last];
