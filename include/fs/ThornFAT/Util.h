...
#define CHECKS(s, e) CHECK(s, "%s: %d", e, status)
...
#if INDENT_WIDTH != 0
#ifdef TRACE_INDENT
#define ENTER { IFLOG { serprintf("\e[A\e[165G" IDS("+%s") "\n", __FUNCTION__); } serprintf(IDS("+%s") "\n", __FUNCTION__); indent(1); }
#define EXIT  { IFLOG { serprintf("\e[A\e[165G" IDS("-%s") "\n", __FUNCTION__); } serprintf(IDS("-%s") "\n", __FUNCTION__); indent(-1); if (indentation[0] == '\0') serprintf("\n"); }
#else
...
inline int IS_X(const char *p, const char *x) {
	auto l = Thorn::FS::ThornFAT::Util::pathLast(p);
